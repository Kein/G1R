...
class G1R_API UAnimNotify_PlayCustomFX : public UAnimNotify_Base {
    GENERATED_BODY()
public:
private:
    ...
    UPROPERTY(Instanced)
    UGothicFXComponent* FXComponent;
    
public:
    UAnimNotify_PlayCustomFX();
...
private:
    ...
    UFUNCTION(BlueprintCallable, BlueprintPure=false)
    void SetFMODEvent(TSoftObjectPtr<UFMODEvent> Event) const;
    
    UFUNCTION(BlueprintPure)
    FGameplayTag GetPerceptionSoundTag() const;
    
    UFUNCTION(BlueprintCallable, BlueprintPure=false)
    TSoftObjectPtr<UFMODEvent> GetFMODEvent() const;
